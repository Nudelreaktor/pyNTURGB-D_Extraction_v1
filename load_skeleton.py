...
def read_skeleton_data( _file_Handler_, _ignore_tail_, _verbose ):
	...
	if( _verbose is True ):
		print( "RSD :: Number of frames in the set: ", number_of_frames )      

	# Cut the first line ( number of frames ) from the raw_Content
	raw_Content = raw_Content[1:]
	if( _verbose is True  ):
		print('RSD :: Number of Skeletons: ', raw_Content[0] ) 
	...
	if( _ignore_tail_ is True ):
		...
		if( noSkel == 1 ):
			...
			if( _verbose is True ):
				print("RSD :: Load_Skel: bef_crop: ", len( frames_of_the_set ))

			frames_of_the_set = frames_of_the_set[:len( frames_of_the_set )-1]

			if( _verbose is True ):
				print("RSD :: Load_Skel: aft_crop: ", len( frames_of_the_set ))

		if( noSkel == 2 ):

			if( _verbose is True ):
				print("RSD :: Load_Skel: bef_crop: ", len( frames_of_the_set ) )
				print("RSD :: Load_Skel: bef_crop2: ", len( frames_2_of_the_set ))
			...
			if( _verbose is True ):
				print("RSD :: Load_Skel: aft_crop: ", len( frames_of_the_set ))
				print("RSD :: Load_Skel: aft_crop2: ", len( frames_2_of_the_set ))
	...
	if( noSkel == 2 ):
		if( lenFrame1 < 2 ):
			if( _verbose == True ):
				print("RSD :: Skeleton 1 has only 1 Frame -> Skeleton 2 is set for the primary one.")
			noSkel = 1
			frames_of_the_set = frames_2_of_the_set

		if( lenFrame2 < 2 ):
			if( _verbose == True ):
				print("RSD :: Skeleton 2 has only 1 Frame -> Skeleton 1 is set for the primary one.")
			noSkel = 1 
	...
	if( noSkel > 1 ): 
		...
		dist_skel_1, dist_skel_2 = check_for_primary_skeleton( frames_of_the_set, frames_2_of_the_set, _verbose )
...
def check_for_primary_skeleton( _frames_of_the_set_, _frames_2_of_the_set_, verbose ):
	...
	if( verbose == True ):
		print( 'CFPS :: #### Moved distance per skeleton ####')
		print( "CFPS :: Dist1: ",dist1)
		print( "CFPS :: Dist2: ",dist2)
		print('\n\n')
